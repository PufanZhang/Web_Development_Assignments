...
所有敏感数据全部的存储、添加逻辑都在后端，如数值系统、成就、手动存档（回档）和游戏时长统计等。在保存数据时，除了玩家位置和地图状态，其他数值都会忽略前端发来的数据，维持服务器上的版本。这样的行为最大程度的防止了前端轻易的篡改数据和通信过程中可能错误的数据被存储。用户密码始终使用 `SHA256` 哈希算法加密并存储在本地，不会向前端泄露用户密码。
